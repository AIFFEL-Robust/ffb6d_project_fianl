...
import math
"""
syn data를 고려 하지 않은 datasert train할때는 아래 add_back함수 부분 활성화 시키고 syn데이터에 대한 처리가 필요 하다 
...
seg_id={1: 12, 2: 24, 3: 36, 4: 48, 5:60, 6:72, 7:84, 8:96,
        9:1, 10:120, 11:132, 12:144, 13:156, 14:168, 15:180,
        16:192, 17:204, 18:3, 19:2, 20:240, 21:252}
# 9:28 
...
def from_json_cls_list(meta):
    cl_lst = []
    for i in range(len(meta['object'])):
        cls_idx = int(meta['object'][i]['class'])
...
class Dataset():
    ...
    def __init__(self, dataset_name, DEBUG=False):
        ...
        if dataset_name == 'train':
            self.add_noise = True
            # test/train data나누는거 물어보기 
            self.path = 'datasets/custom/dataset_config/train_data_list.txt'
            ...
            # 바꾸기
            self.path = 'datasets/custom/dataset_config/test_data_list.txt'
            self.all_lst = bs_utils.read_lines(self.path)
        print("{}_dataset_size: ".format(dataset_name), len(self.all_lst))
        self.root = config.custom_root # custom dataset root를 config에서 설정하였다
...
#         # png를 .color.png으로 바꾸어야 한다 -> 뎁스 라벨 다 가져옴 
...
#             print(dpt_um.max(),dpt_um.min())
        ...
        # custom dataset camera intrinsic matrix -> config file에 추가 해주어도 될것 같다 
        #우선은 직접 할당 
        K = np.array([[593.695556,0.,315.979554],
                      [0.,593.695556,238.561188],
                      [0.,0.,1.]])
        ...
        # # noise 추가 부분 
        # with Image.open(os.path.join(self.root, item_name+'.png')) as ri:
        #     if self.add_noise:
        #         ri = self.trancolor(ri)
        #     rgb = np.array(ri)[:, :, :3]
            # print(item_name,"rgb noise done")
...
#         print("---------------------------------------------")
    ...
    def get_pose_gt_info(self, cld, labels, cls_id_lst, meta):
        ...
        for i, cls_id in enumerate(cls_id_lst):
            ...
            # r, t = cal_rt(meta, i, cls_id)
            
            # custom dataset rt
            info = meta["object"][i]
            
            r = np.array(info["rotation"]) # numpy array 3,3
            t = np.array(info["translation"]) # numpy array 3,  -> new axis
            t=t[...,None] # (3,1) for caculate RT 
            ...
            RT = np.concatenate((r, t), axis=1) # 3*4 numpy array 
            RTs[i] = RT
...
# 데이터 키포인트 등 을 시각화 한다 한번 확인은 해봐도 좋을듯 하다 
def main():
