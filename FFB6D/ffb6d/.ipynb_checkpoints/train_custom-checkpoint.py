...
def view_labels(rgb_chw, cld_cn, labels, K=config.intrinsic_matrix['custom']):
...
class Trainer(object):
    ...
    def eval_epoch(self, d_loader, is_test=False, test_pose=False, it=0):
        ...
        if is_test:
            ...
            seg_res_fn = 'seg_res'
            for iidx,(k, v) in enumerate(acc_dict.items()):
                seg_res_fn += '_%s%.2f' % (k, iidx)
...
def train():
    ...
    #-----------model parameters Freeze and Activation layer ----------------------------------
    # require grad만 freeze하는거라서 model.loade_state()적용해도 이상은 없다 
    
    param_idx=1
    for param in model.parameters():
        param.requires_grad = False
        if param_idx>322 and param_idx<356:
    ...
    #----------------summery model로 trainable한지 확인하기 --------------------
    print("DEBUG : Check the requires Grad ")
    print()
    param_idx2=1
    for name,param in model.named_parameters():
        print(param_idx2,name,param.requires_grad)
        param_idx2+=1
...
# #         print(name,param.requires_grad)
    ...
    bnm_lmbd = lambda it: max(
        args.bn_momentum * args.bn_decay ** (int(it * 1 / args.decay_step)),
        bnm_clip,
    )
