...
import datasets.ycb.ycb_dataset_ycbmfatsyn as dataset_desc
...
def model_fn_decorator(
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            loss_lst = [
                (loss_rgbd_seg, 2.0), (loss_kp_of, 2.0), (loss_ctr_of, 1.0),
            ]
    ...
    def train(
        ...
        with tqdm.tqdm(range(config.n_total_epoch), desc="epochs") as tbar, tqdm.tqdm(
            ...
            for epoch in tbar:
                ...
                # epoch별 checkpoint save
                save_checkpoint(checkpoint_state(self.model, self.optimizer, val_loss, epoch, it), 
                        is_best, 
                        filename=self.checkpoint_name + '_' + str(epoch) + '_epoch_' + str(it) + '_iter', 
                        bestname=self.best_name+'_%.4f' % val_loss, 
                        bestname_pure=self.best_name)
...
def train():
    ...
    if args.checkpoint is not None:
        ...
        if checkpoint_status is not None:
            ...
            # prt모델로 트랜스퍼 러닝할때 best 모델을 저장하기 위한 방법
            best_loss += 20
        if args.eval_net:
