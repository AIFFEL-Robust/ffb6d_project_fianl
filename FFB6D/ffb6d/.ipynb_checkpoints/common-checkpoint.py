...
class ConfigRandLA:
    ...
    batch_size = 7  # batch_size during training
...
class Config:
    def __init__(self, ds_name='ycb', cls_type=''):
        ...
        if self.dataset_name == 'ycb':
            ...
            # custom dataset root
            self.custom_root = os.path.abspath(
                os.path.join(
                    self.exp_dir,'datasets/custom/Custom_Dataset'
                )
            )
        ...
        # YCBM 및 custom data를 위한 camera intrinsic
        self.intrinsic_matrix = {
            ...
            'ycb_K2': np.array([[1077.836, 0.        , 323.7872],
                                [0.      , 1078.189  , 279.6921],
                                [0.      , 0.        , 1.0]], np.float32),
            'xtion': np.array([[540.353233318621, 0.        , 303.266572921124],
                                [0.      , 545.07810999676  , 261.674552167447],
                                [0.      , 0.        , 1.0]], np.float32),
            'astra': np.array([[518.207829489907, 0.                , 313.528541176162],
                                [0.             , 515.956597204714  , 245.157107506499],
                                [0.             , 0.                , 1.              ]], np.float32),
            'realsense': np.array([[618.140991210937, 0.        , 311.465911865234],
                                [0.      , 618.140991210937  , 236.292694091796],
                                [0.      , 0.        , 1.0]], np.float32),
            'kinect2': np.array([[537.7640178514, 0.        , 481.597069110053],
                                [0.      , 533.319583171145  , 273.286537981435],
                                [0.      , 0.        , 1.0]], np.float32),
            'enseneo': np.array([[1139.34439475924, 0.        , 606.56050839291],
                                [0.      , 1139.34439475924  , 503.498708286859],
                                [0.      , 0.        , 1.0]], np.float32),
            'custom':np.array([[593.695556,0.,315.979554],
